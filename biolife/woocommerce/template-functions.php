	...
	function ovic_action_after_setup_theme()
	{
		...
		if ( class_exists( 'YITH_WCQV_Frontend' ) ) {
			...
			if ( ( !wp_is_mobile() && $enable ) || ( wp_is_mobile() && $enable_on_mobile && $enable ) ) {
				...
				add_action( 'ovic_function_shop_loop_item_quickview', array( YITH_WCQV_Frontend::get_instance(), 'yith_add_quick_view_button' ), 5 );
			}
		}
		/* WISH LIST */
		if ( defined( 'YITH_WCWL' ) ) {
			add_action( 'ovic_function_shop_loop_item_wishlist', function () { echo do_shortcode( "[yith_wcwl_add_to_wishlist]" ); }, 1 );
		}
		/* COMPARE */
	...
	function ovic_action_attributes()
	{
		...
		if ( $product->get_type() == 'variable' ) :
			...
			foreach ( $available_variations as $available_variation ) {
				$image_variable                            = apply_filters( 'ovic_resize_image', $available_variation['image_id'], $width, $height, $crop, false );
				...
				$available_variation['image']['srcset']    = $image_variable['url'];
						...
						<?php foreach ( $attributes as $attribute_name => $options ) : ?>
                            <tr>
                                ...
                                <td class="value">
									...
									echo end( $attribute_keys ) === $attribute_name ? apply_filters( 'woocommerce_reset_variations_link', '<a class="reset_variations" href="#">' . esc_html__( 'Clear', 'biolife' ) . '</a>' ) : '';
									?>
                                </td>
                            </tr>
						<?php endforeach; ?>
...
if ( !function_exists( 'ovic_woocommerce_before_loop_content' ) ) {
	function ovic_woocommerce_before_loop_content()
	{
		...
		echo wp_specialchars_decode('<div class="' . esc_attr( implode( ' ', $main_content_class ) ) . '">');
	}
}
...
if ( !function_exists( 'ovic_product_get_rating_html' ) ) {
	function ovic_product_get_rating_html( $html, $rating, $count )
	{
		...
		if ( 0 < $rating ) {
			...
			$html .= '</div>';
			if ( $rating_count > 0 ) {
				$html .= '<span class="review">( ' . esc_html($rating_count) . ' ' . esc_html__( 'review', 'biolife' ) . ' )</span>';
			}
			$html .= '</div>';
		} else {
			$html = '';
            ...
            <form method="get">
                ...
                <button type="submit"
                        ...
                        <span class="button-inner">
                            <?php echo esc_html__( 'Grid', 'biolife' ); ?>
                        </span>
                </button>
                <button type="submit"
                        ...
                        name="ovic_shop_list_style">
                        <span class="button-inner">
                            <?php echo esc_html__( 'List', 'biolife' ); ?>
                        </span>
                </button>
...
if ( !function_exists( 'ovic_loop_shop_per_page' ) ) {
	...
	{
		$ovic_woo_products_perpage = intval(apply_filters( 'ovic_get_option', 'ovic_product_per_page', '12' ));
		if(!$ovic_woo_products_perpage)
            $ovic_woo_products_perpage = 12;
...
if ( !function_exists( 'ovic_product_per_page_tmp' ) ) {
	function ovic_product_per_page_tmp()
	{
		...
		$perpage = intval(apply_filters( 'ovic_get_option', 'ovic_product_per_page', '12' ));
		if (!$perpage)
            $perpage = 12;
		?>
	...
	function ovic_custom_pagination()
	{
		...
		if ( $wp_query->max_num_pages > 1 ) {
            ...
            <nav class="woocommerce-pagination pagination">
				<?php
				echo paginate_links( apply_filters( 'woocommerce_pagination_args', array( 'base' => esc_url_raw( str_replace( 999999999, '%#%', remove_query_arg( 'add-to-cart', get_pagenum_link( 999999999, false ) ) ) ), 'format' => '', 'add_args' => false, 'current' => max( 1, get_query_var( 'paged' ) ), 'total' => $wp_query->max_num_pages, 'prev_text' => esc_html__( 'Previous', 'biolife' ), 'next_text' => esc_html__( 'Next', 'biolife' ), 'type' => 'plain', 'end_size' => 3, 'mid_size' => 3, ) ) );
				?>
            </nav>
			<?php
		}
...
if ( !function_exists( 'ovic_carousel_products' ) ) {
	function ovic_carousel_products( $prefix, $data_args )
	{
		...
		$classes_contain[]      = ovic_generate_class_nav( 'owl_', $atts, count( $data_args ) );
...
if ( !function_exists( 'ovic_related_products' ) ) {
	...
	{
		global $product;
		$defaults                    = array( 'posts_per_page' => 6, 'columns' => 6, 'orderby' => 'rand', 'order' => 'desc', );
		...
		ovic_carousel_products( 'ovic_woo_related', $related_products );
	}
}
...
if ( !function_exists( 'ovic_template_loop_product_thumbnail' ) ) {
	function ovic_template_loop_product_thumbnail()
	{
		...
		$width       = apply_filters( 'ovic_shop_product_thumb_width', $width );
		$height      = apply_filters( 'ovic_shop_product_thumb_height', $height );
		$image_thumb = apply_filters( 'ovic_resize_image', get_post_thumbnail_id( $product->get_id() ), $width, $height, $crop, true );
		?>
...
if ( !function_exists( 'ovic_custom_new_flash' ) ) {
	function ovic_custom_new_flash()
	{
		...
		if ( ( time() - ( 60 * 60 * 24 * $newness ) ) < $postdatestamp ) :
			echo apply_filters( 'woocommerce_new_flash', '<span class="onnew"><span class="text">' . esc_html__( 'New', 'biolife' ) . '</span></span>', $post, $product );
		endif;
	}
}
...
if ( !function_exists( 'ovic_custom_sale_flash' ) ) {
	function ovic_custom_sale_flash( $text )
	{
		...
		if ( $percent != '' )
			return '<span class="onsale"><span class="text">' . esc_html($percent) . '</span></span>';
