...
switch ( $source ) {
	...
	case 'featured_image':
		...
		$image_thumb      = '<img class="vc_single_image-img ' . $lazy_load_class . ' attachment-' . $img_size . '" src="' . $lazy_url . '" ' . $data_src . ' ' . $hwstring . ' alt="' . esc_attr(get_the_title()) . '"/>';
		$img              = array(
	...
	case 'external_link':
		...
		$img              = array(
			'thumbnail' => '<img class="vc_single_image-img ' . $lazy_load_class . '" ' . $hwstring . ' src="' . $lazy_url . '" ' . $data_src . ' alt="' . esc_attr(get_the_title()) . '"/>',
		);
		break;
	default:
		$img = false;
...
if ( $title ) {
	$title_html = '
	<h2 class="supermarket-title wpb_heading wpb_singleimage_heading">
