...
<?php
    ...
    if ( $customer_orders ) : ?>
        <h2 style="margin-top: 30px;margin-bottom: 20px;"><?php echo esc_html( apply_filters( 'woocommerce_my_account_my_orders_title', __( 'Recent Orders', 'biolife' ) ) ); ?></h2>
        <table class="shop_table my_account_orders table table-striped">

            <thead>
                <tr>
                    <th class="order-number"><span class="nobr"><?php esc_html_e( 'Order', 'biolife' ); ?></span></th>
                    <th class="order-date"><span class="nobr"><?php esc_html_e( 'Date', 'biolife' ); ?></span></th>
                    <th class="order-status"><span class="nobr"><?php esc_html_e( 'Status', 'biolife' ); ?></span></th>
                    <th class="order-total"><span class="nobr"><?php esc_html_e( 'Total', 'biolife' ); ?></span></th>
                    <th class="order-total"><span class="nobr"><?php esc_html_e( 'Vendor', 'biolife' ); ?></span></th>
                ...
                foreach ( $customer_orders as $customer_order ) {
                    ...
                    ?><tr class="order">
                        ...
                        <td class="order-total">
                            <?php echo wp_kses_post( sprintf( _n( '%s for %s item', '%s for %s items', $item_count, 'biolife' ), $order->get_formatted_order_total(), $item_count ) ); ?>
                        </td>
                        ...
                        <td class="order-total">
                            <?php
                                ...
                                if ( !is_array( $seller_id ) && $seller_id != 0 ) {
                                    ...
                                    esc_html_e( 'Multiple Vendor', 'biolife' );
                                }
                            ?>
                        </td>
                            ...
                            <?php
                                ...
                                if ( in_array( dokan_get_prop( $order, 'status' ), apply_filters( 'woocommerce_valid_order_statuses_for_cancel', array( 'pending', 'failed' ), $order ) ) )
                                    $actions['cancel'] = array(
                                        ...
                                        'name' => __( 'Cancel', 'biolife' )
                                    );

                                $actions['view'] = array(
                                    'url'  => $order->get_view_order_url(),
                                    'name' => __( 'View', 'biolife' )
                                );
    ...
    <?php else: ?>
        ...
        <p class="dokan-info"><?php esc_html_e( 'No orders found!', 'biolife' ); ?></p>
