...
<tr class="<?php echo esc_attr( $tr_class ); ?>">
    ...
    <td data-title="<?php esc_attr_e( 'Image', 'biolife' ); ?>">
    ...
    <td data-title="<?php esc_attr_e( 'Name', 'biolife' ); ?>">
        <?php if ( current_user_can( 'dokan_edit_product' ) ): ?>
    ...
    <td class="post-status" data-title="<?php esc_attr_e( 'Status', 'biolife' ); ?>">
    ...
    <td data-title="<?php esc_attr_e( 'SKU', 'biolife' ); ?>">
        <?php
        if ( $product->get_sku() ) {
    ...
    <td data-title="<?php esc_attr_e( 'Stock', 'biolife' ); ?>">
        <?php
    ...
    <td data-title="<?php esc_attr_e( 'Price', 'biolife' ); ?>">
        <?php
    ...
    <td data-title="<?php esc_attr_e( 'Earning', 'biolife' ); ?>">
        <?php
    ...
    <td data-title="<?php esc_attr_e( 'Type', 'biolife' ); ?>">
        <?php
        if( dokan_get_prop( $product, 'product_type' , 'get_type') == 'grouped' ):
            echo '<span class="product-type tips grouped" title="' . esc_html__( 'Grouped', 'biolife' ) . '"></span>';
        elseif ( dokan_get_prop( $product, 'product_type' , 'get_type') == 'external' ):
            echo '<span class="product-type tips external" title="' . esc_html__( 'External/Affiliate', 'biolife' ) . '"></span>';
        elseif ( dokan_get_prop( $product, 'product_type' , 'get_type') == 'simple' ):

            if ( $product->is_virtual() ) {
                echo '<span class="product-type tips virtual" title="' . esc_html__( 'Virtual', 'biolife' ) . '"></span>';
            } elseif ( $product->is_downloadable() ) {
                echo '<span class="product-type tips downloadable" title="' . esc_html__( 'Downloadable', 'biolife' ) . '"></span>';
            } else {
                echo '<span class="product-type tips simple" title="' . esc_html__( 'Simple', 'biolife' ) . '"></span>';
            }

            elseif ( dokan_get_prop( $product, 'product_type' , 'get_type') == 'variable' ):
                echo '<span class="product-type tips variable" title="' . esc_html__( 'Variable', 'biolife' ) . '"></span>';
            else:
    ...
    <td data-title="<?php esc_attr_e( 'Views', 'biolife' ); ?>">
        <?php echo (int) get_post_meta( $post->ID, 'pageview', true ); ?>
    </td>
    <td class="post-date" data-title="<?php esc_attr_e( 'Date', 'biolife' ); ?>">
        <?php
        if ( '0000-00-00 00:00:00' == $post->post_date ) {
            $t_time = $h_time = __( 'Unpublished', 'biolife' );
            $time_diff = 0;
        } else {
            $t_time = get_the_time( __( 'Y/m/d g:i:s A', 'biolife' ) );
            ...
            if ( $time_diff > 0 && $time_diff < 24 * 60 * 60 ) {
                $h_time = sprintf( __( '%s ago', 'biolife' ), human_time_diff( $time ) );
            } else {
                $h_time = mysql2date( __( 'Y/m/d', 'biolife' ), $m_time );
            }
        }
        ...
        if ( 'publish' == $post->post_status ) {
            esc_html_e( 'Published', 'biolife' );
        } elseif ( 'future' == $post->post_status ) {
            if ( $time_diff > 0 ) {
                echo '<strong class="attention">' . esc_html__( 'Missed schedule', 'biolife' ) . '</strong>';
            } else {
                esc_html_e( 'Scheduled', 'biolife' );
            }
        } else {
            esc_html_e( 'Last Modified', 'biolife' );
        }
        ?>
        </div>
    </td>
