...
if ( !class_exists( 'Ovic_Shortcode_Instagram' ) ) {
	class Ovic_Shortcode_Instagram extends Ovic_Shortcode
	{
		...
		public function output_html( $atts, $content = null )
		{
			...
			// Extract shortcode parameters.
			extract( $atts );
			$css_class    = array( 'ovic-instagram', biolife_getCSSAnimation( $css_animation ) );
			...
			$instagram_list_class = array( 'content-instagram', $layout );
			$owl_settings         = '';
			$items                = array();
			ob_start();
			?>
            <div class="<?php echo esc_attr( implode( ' ', $css_class ) ); ?>">
                <div class="title-container">
					<?php if ( $atts['layout'] == 'default' ) : ?>
						<?php if ( isset( $atts['sub_title'] ) && $atts['sub_title'] ) : ?>
                            <div class="sub_title"><?php echo esc_html( $atts['sub_title'] ); ?></div>
						<?php endif; ?>
						<?php if ( isset( $atts['title'] ) && $atts['title'] ) : ?>
                            <div class="box-title"><?php echo esc_html( $atts['title'] ); ?></div>
						<?php endif; ?>
						<?php if ( isset( $atts['desc'] ) && $atts['desc'] ) : ?>
                            <div class="box-desc"><?php echo esc_html( $atts['desc'] ); ?></div>
						<?php endif; ?>
					<?php elseif ( $atts['layout'] == 'style1' ) : ?>
						<?php if ( isset( $atts['title'] ) && $atts['title'] ) : ?>
                            <div class="box-title"><?php echo esc_html( $atts['title'] ); ?></div>
						<?php endif; ?>
				...
				if ( $atts['image_source'] == 'instagram' ) {
					...
					if ( intval( $atts['id_instagram'] ) === 0 || intval( $atts['token'] ) === 0 ) {
						echo sprintf( '<div class="alert alert-warning"><strong>%s</strong> %s</div>',
							...
							esc_html__( 'No user ID specified.', 'biolife' )
						);
					}
					if ( empty( $instagram ) || $instagram === false && $atts['id_instagram'] && $atts['token'] ) {
						...
						if ( !is_wp_error( $response ) ) {
							...
							if ( $response_code != 200 ) {
								echo sprintf( '<div class="alert alert-warning"><strong>%s</strong> %s</div>',
									esc_html__( 'Warning!', 'biolife' ),
									esc_html__( 'User ID and access token do not match. Please check again.', 'biolife' )
								);
							} else {
				...
				if ( isset( $items ) && $items ): ?>
                    <div class="<?php echo implode( ' ', $instagram_list_class ); ?>" <?php echo esc_attr( $owl_settings ); ?>>
						<?php foreach ( $items as $item ):
							if ( isset( $item['images'][$atts['image_resolution']] ) && !empty( $item['images'][$atts['image_resolution']] ) ):
                                ...
                                <div class="<?php echo implode( ' ', $instagram_item_class ); ?>">
                                    <a href="<?php echo esc_url( $item['link'] ) ?>" class="thumb-instagram">
                                        <figure>
