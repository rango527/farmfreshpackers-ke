    ...
    class Ovic_Import_Database_Sample_Data
    {
        ...
        public static function download($uri, $target = '')
        {
            ...
            if (!empty($target)) {
                ...
                // Move file.
                WP_Filesystem_Direct::copy($result, $target, true);

                $content = ($content = filesize($target)) ? $content : WP_Filesystem_Direct::size($target);
            } else {
                $content = ($content = file_get_contents($result)) ? $content : WP_Filesystem_Direct::get_contents($result);

                // Remove downloaded file.
                unlink($result) || WP_Filesystem_Direct::delete($result);
            }
