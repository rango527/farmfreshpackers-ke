	...
	var otpForm = function( phoneFormHandler ){
		...
		if( self.displayType === 'external_form' ){
			...
			if( !self.phoneFormHandler.$phoneInput.siblings('.xoo-ml-inline-otp-cont').length ){
				$( xoo_ml_phone_localize.html.otp_form_inline ).insertAfter( self.phoneFormHandler.$phoneInput );
			}
			self.$otpForm = self.phoneFormHandler.$phoneInput.siblings('.xoo-ml-inline-otp-cont');
