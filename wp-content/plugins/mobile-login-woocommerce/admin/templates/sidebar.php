<?php
	$plugins_list = array(
		...
		array(
			...
			'desc' 		=> 'Login/signup anywhere from the site with the simple pop up without refreshing page.',
		...
		array(
			...
			'download'  => 'https://wordpress.org/plugins/user-verification-for-woocommerce/'
		),
	...
	<ul class="xoo-op-list">
		<?php foreach($plugins_list as $plugin): ?>
			...
			<?php if( isset( $plugin['type'] ) && $plugin['type'] === 'section' ): ?>
				<li class="xoo-sidebar-head section-<?php echo $plugin['id']; ?>"><?php echo $plugin['title']; ?></li>
			<?php continue; endif; ?>
