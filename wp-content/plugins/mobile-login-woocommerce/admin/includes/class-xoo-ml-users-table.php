				...
				<td>
					<select name="xoo-ml-user-reg-phone-cc">
						...
						<?php foreach( $phoneCodes as $country_code => $country_phone_code ): ?>
							<option value="<?php echo $country_phone_code; ?>" <?php echo $country_phone_code === get_user_meta( $user->ID, 'xoo_ml_phone_code',true) ? 'selected' : ''; ?> ><?php echo $country_code.' '.$country_phone_code; ?></option>
						<?php endforeach; ?>
				</select>
					<input type="text" name="xoo-ml-user-reg-phone" value="<?php echo get_user_meta( $user->ID, 'xoo_ml_phone_no',true); ?>">
				</td>
			</tr>
		</table>
		<?php
