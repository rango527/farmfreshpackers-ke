...
class Xoo_Ml_Admin_Settings{
	...
	public function __construct(){
		...
		if( current_user_can( 'manage_options' ) ){
			add_action( 'wp_ajax_download_easylogin_plugin', array( $this, 'download_easylogin_plugin' ) );
	...
	public function generate_settings( $setting_fields, $page, $group, $option_name ){
		...
		foreach ($setting_fields as $field) {
			...
			if( !isset($field['id']) || !isset($field['type']) || !isset($field['callback'])  || ( isset( $field['pro'] ) && $field['pro'] === "yes" ) ) {
				continue;
			}
	...
	public function download_easylogin_plugin(){
		...
		try {
			...
			//Download
			$permfile 		= WP_PLUGIN_DIR.'/easy-login-woocommerce.zip';
			$tmpfile 		= download_url( 'https://downloads.wordpress.org/plugin/easy-login-woocommerce.zip', $timeout = 300 );
