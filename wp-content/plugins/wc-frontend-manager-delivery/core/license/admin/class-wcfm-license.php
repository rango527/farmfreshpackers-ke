...
class WCFM_License {
	...
	public function __construct($tab) {
		...
		add_menu_page(
				__('WCFM License', 'wc-frontend-manager-delivery'), 
				__('WCFM License', 'wc-frontend-manager-delivery'), 
				'manage_options', 
				'wcfm-license', 
				array( $this, 'create_wcfm_license_settings' ),
				$WCFMd->plugin_url . 'assets/images/wcfm_ico.png'
		);
	}
  ...
  function wcfm_license_tabs( $current = '' ) {
    ...
    foreach( $this->get_wcfm_license_tabs() as $tab => $name ) :
      if ( $tab == $current ) :
        printf( __( "<h2>%s License</h2>", 'wc-frontend-manager-delivery') , $name);
      endif;
    endforeach;
    ...
    foreach( $this->get_wcfm_license_tabs() as $tabd => $named ) :
      if ( $tabd == $current && !empty($tab_desc[$tabd]) ) :
        printf( __( "<h4 style='border-bottom: 1px solid rgb(215, 211, 211);padding-bottom: 21px;'>%s</h4>", 'wc-frontend-manager-delivery') , $tab_desc[$tabd]);
      endif;
    endforeach;
  }
  ...
  public function create_wcfm_license_settings() {
    ...
    <div class="wrap">
      ...
      <?php
      $tab = ( isset( $_GET['tab'] ) ? $_GET['tab'] : 'wcfmd_license' );
	...
	*/
	public function enqueue_admin_script() {
		...
		if (in_array( $screen->id, array( 'toplevel_page_wcfm-license' ))) : 
		  wp_enqueue_style('wcfm_admin_css',  $WCFM->plugin_url.'assets/admin/css/admin.css', array(), $WCFMd->version);
		endif;
	}
