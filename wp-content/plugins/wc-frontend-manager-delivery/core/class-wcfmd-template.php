...
/**
 * @class 		WCFMd Template Class
 *
 * @version		1.0.0
 * @package		WC
 * @author 		WC Lovers
 */
class WCFMd_Template {

    public $template_url;

    public function __construct() {
        $this->template_url = 'wcfm/delivery/';
    }
    ...
    public function get_template_part($slug, $name = '') {
        ...
        if (!$template && $name && file_exists($WCFMd->plugin_path . "views/{$slug}-{$name}.php"))
            $template = $WCFMd->plugin_path . "views/{$slug}-{$name}.php";
