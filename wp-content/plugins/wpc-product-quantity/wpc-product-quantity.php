...
! defined( 'WOOPQ_VERSION' ) && define( 'WOOPQ_VERSION', '1.3.1' );
			...
			class WPCleverWoopq {
				function __construct() {
					...
					add_filter( 'woocommerce_add_to_cart_validation', array(
						$this,
						'woopq_add_to_cart_validation'
					), 10, 3 );

					// add to cart message
					if ( get_option( '_woopq_decimal', 'no' ) === 'yes' ) {
						add_filter( 'wc_add_to_cart_message_html', array(
							$this,
							'woopq_add_to_cart_message_html'
						), 99, 3 );
					}
				}
				...
				function woopq_quantity_input_args( $args, $product ) {
					if ( $product ) {
						...
						$args['min_value']  = $this->woopq_get_product_min( $product_id, $args['min_value'] );
						$args['max_value']  = $this->woopq_get_product_max( $product_id, $args['max_value'] );
						$args['step']       = $this->woopq_get_product_step( $product_id, $args['step'] );
					}
				...
				function woopq_quantity_input_min( $min, $product ) {
					if ( $product ) {
						...
						return $this->woopq_get_product_min( $product_id, $min );
					}
				...
				function woopq_quantity_input_max( $max, $product ) {
					if ( $product ) {
						...
						return $this->woopq_get_product_max( $product_id, $max );
					}
				...
				function woopq_quantity_input_step( $step, $product ) {
					if ( $product ) {
						...
						return $this->woopq_get_product_step( $product_id, $step );
					}

					return $step;
				}

				function woopq_get_product_min( $product_id, $min = 0 ) {
					$woopq_quantity = get_post_meta( $product_id, '_woopq_quantity', true ) ?: 'default';
					$woopq_min      = (float) get_post_meta( $product_id, '_woopq_min', true );
				...
				function woopq_get_product_max( $product_id, $max = 100000 ) {
					$woopq_quantity = get_post_meta( $product_id, '_woopq_quantity', true ) ?: 'default';
				...
				function woopq_get_product_step( $product_id, $step = 1 ) {
					$woopq_quantity = get_post_meta( $product_id, '_woopq_quantity', true ) ?: 'default';
				...
				function woopq_add_to_cart_validation( $passed, $product_id, $quantity ) {
					$max   = $this->woopq_get_product_max( $product_id );
					$added = $this->woopq_qty_in_cart( $product_id );

					if ( ( $quantity + $added ) > $max ) {
				...
				function woopq_qty_in_cart( $product_id ) {
					$qty = 0;
				...
				function woopq_add_to_cart_message_html( $message, $products, $show_qty ) {
					$titles = array();
					$count  = 0;
