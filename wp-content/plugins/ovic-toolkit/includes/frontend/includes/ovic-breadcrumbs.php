...
if (!class_exists('Breadcrumb_Trail')) {
    class Breadcrumb_Trail
    {
        ...
        {
            add_action('ovic_breadcrumb', array($this, 'ovic_breadcrumb'), 1);
        }

        function ovic_breadcrumb($args = array())
        {
            $defaults = array(
                'container' => 'nav',
                'before' => '',
        ...
        public function trail()
        {
            ...
            if (0 < $item_count) {
                ...
                $breadcrumb .= '<ul class="trail-items breadcrumb" xmlns:v="'.esc_url('http://rdf.data-vocabulary.org/#').'">';
                ...
                foreach ($this->items as $item) {
                    ...
                    $item = !empty($matches) ? sprintf('%s<span class="name">%s</span>%s', $matches[1], $matches[2],
                        $matches[3]) : sprintf('<span class="name">%s</span>', $item);
                    ...
                    $attributes = 'class="'.$item_class.'" typeof="v:Breadcrumb"';
        ...
        {
            $defaults = array(
                'browse' => esc_html__('Browse:', 'ovic-toolkit'),
                'aria_label' => esc_attr_x('Breadcrumbs', 'breadcrumbs aria label', 'ovic-toolkit'),
                'home' => esc_html__('Home', 'ovic-toolkit'),
                'error_404' => esc_html__('404 Not Found', 'ovic-toolkit'),
                'archives' => esc_html__('Archives', 'ovic-toolkit'),
                // Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
                'search' => esc_html__('Search results for &#8220;%s&#8221;', 'ovic-toolkit'),
                // Translators: %s is the page number.
                'paged' => esc_html__('Page %s', 'ovic-toolkit'),
                // Translators: Minute archive title. %s is the minute time format.
                'archive_minute' => esc_html__('Minute %s', 'ovic-toolkit'),
                // Translators: Weekly archive title. %s is the week date format.
                'archive_week' => esc_html__('Week %s', 'ovic-toolkit'),
        ...
        protected function set_post_taxonomy()
        {
            ...
            $this->post_taxonomy = apply_filters('breadcrumb_trail_post_taxonomy',
                wp_parse_args($this->args['post_taxonomy'], $defaults));
        }
        ...
        function add_network_home_link()
        {
            if (is_multisite() && !is_main_site() && true === $this->args['network']) {
                $this->items[] = sprintf('<a href="%s" rel="home">%s</a>', esc_url(network_home_url()),
                    $this->labels['home']);
            }
        }
        ...
        protected function add_posts_page_items()
        {
            ...
            if (is_paged()) {
                $this->items[] = sprintf('<a rel="v:url" property="v:title" title="" href="%s">%s</a>',
                    esc_url(get_permalink($post_id)), $title);
        ...
        protected function add_singular_items()
        {
            ...
            if ($post_title = single_post_title('', false)) {
                if (1 < get_query_var('page') || is_paged()) {
                    $this->items[] = sprintf('<a rel="v:url" property="v:title" title="" href="%s">%s</a>',
                        esc_url(get_permalink($post_id)), $post_title);
                } elseif (true === $this->args['show_title']) {
                ...
                if ($taxonomy->rewrite['slug']) {
                    ...
                    if (isset($matches)) {
                        ...
                        foreach ($matches as $match) {
                            ...
                            $post_types = self::get_post_types_by_slug($slug);
                            if (!empty($post_types)) {
                                ...
                                $this->items[] = sprintf('<a rel="v:url" property="v:title" title="" href="%s">%s</a>',
                                    esc_url(get_post_type_archive_link($post_type_object->name)), $label);
                                $done_post_type = true;
        ...
        protected function add_post_type_archive_items()
        {
            ...
            if (is_paged()) {
                $this->items[] = sprintf('<a rel="v:url" property="v:title" title="" href="%s">%s</a>',
                    esc_url(get_post_type_archive_link($post_type_object->name)), post_type_archive_title('', false));
            } elseif (true === $this->args['show_title']) {
        ...
        protected function add_minute_hour_archive_items()
        {
            ...
            if (true === $this->args['show_title']) {
                $this->items[] = sprintf($this->labels['archive_minute_hour'],
                    get_the_time(esc_html_x('g:i a', 'minute and hour archives time format', 'ovic-toolkit')));
            }
        }
        ...
        protected function add_minute_archive_items()
        {
            ...
            if (true === $this->args['show_title']) {
                $this->items[] = sprintf($this->labels['archive_minute'],
                    get_the_time(esc_html_x('i', 'minute archives time format', 'ovic-toolkit')));
            }
        }
        ...
        protected function add_hour_archive_items()
        {
            ...
            if (true === $this->args['show_title']) {
                $this->items[] = sprintf($this->labels['archive_hour'],
                    get_the_time(esc_html_x('g a', 'hour archives time format', 'ovic-toolkit')));
            }
        }
        ...
        protected function add_day_archive_items()
        {
            ...
            $year = sprintf($this->labels['archive_year'],
                get_the_time(esc_html_x('Y', 'yearly archives date format', 'ovic-toolkit')));
            $month = sprintf($this->labels['archive_month'],
                get_the_time(esc_html_x('F', 'monthly archives date format', 'ovic-toolkit')));
            $day = sprintf($this->labels['archive_day'],
                get_the_time(esc_html_x('j', 'daily archives date format', 'ovic-toolkit')));
            // Add the year and month items.
            $this->items[] = sprintf('<a rel="v:url" property="v:title" title="" href="%s">%s</a>',
                esc_url(get_year_link(get_the_time('Y'))), $year);
            $this->items[] = sprintf('<a rel="v:url" property="v:title" title="" href="%s">%s</a>',
                esc_url(get_month_link(get_the_time('Y'), get_the_time('m'))), $month);
            // Add the day item.
            if (is_paged()) {
                $this->items[] = sprintf('<a rel="v:url" property="v:title" title="" href="%s">%s</a>',
                    esc_url(get_day_link(get_the_time('Y'), get_the_time('m'), get_the_time('d')), $day));
            } elseif (true === $this->args['show_title']) {
                $this->items[] = $day;
            }
        }
        ...
        protected function add_week_archive_items()
        {
            ...
            $week = sprintf($this->labels['archive_week'],
                get_the_time(esc_html_x('W', 'weekly archives date format', 'ovic-toolkit')));
            // Add the year item.
            $this->items[] = sprintf('<a rel="v:url" property="v:title" title="" href="%s">%s</a>',
                esc_url(get_year_link(get_the_time('Y'))), $year);
            // Add the week item.
            if (is_paged()) {
                $this->items[] = esc_url(get_archives_link(add_query_arg(array(
                    'm' => get_the_time('Y'), 'w' => get_the_time('W')
                ), home_url()), $week, false));
            } elseif (true === $this->args['show_title']) {
                $this->items[] = $week;
            }
        ...
        protected function add_search_items()
        {
            if (is_paged()) {
                $this->items[] = sprintf('<a rel="v:url" property="v:title" title="" href="%s">%s</a>',
                    ...
                    sprintf($this->labels['search'], get_search_query()));
            } elseif (true === $this->args['show_title']) {
        ...
        protected function add_post_parents($post_id)
        {
            ...
            while ($post_id) {
                ...
                $parents[] = sprintf('<a rel="v:url" property="v:title" title="" href="%s">%s</a>',
                    esc_url(get_permalink($post_id)), get_the_title($post_id));
        ...
        protected function add_post_hierarchy(
            $post_id
        ) {
            // Get the post type.
        ...
        protected function get_post_types_by_slug(
            $slug
        ) {
            $return = array();
        ...
        protected function add_post_terms(
            ...
            if ($terms && !is_wp_error($terms)) {
                ...
                $this->items[] = sprintf('<a rel="v:url" property="v:title" title="" href="%s">%s</a>',
                    esc_url(get_term_link($term, $taxonomy)), $term->name);
            }
        }
        ...
        protected function map_rewrite_tags(
            $post_id,
            $path
        ) {
            $post = get_post($post_id);
            ...
            if (is_array($matches)) {
                ...
                foreach ($matches as $match) {
                    ...
                    if ('%year%' == $tag) {
                        $this->items[] = sprintf('<a rel="v:url" property="v:title" title="" href="%s">%s</a>',
                            esc_url(get_year_link(get_the_time('Y', $post_id))), sprintf($this->labels['archive_year'],
                                get_the_time(esc_html_x('Y', 'yearly archives date format', 'ovic-toolkit'))));
