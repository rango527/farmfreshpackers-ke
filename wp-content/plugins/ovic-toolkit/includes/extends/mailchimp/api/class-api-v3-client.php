<?php
if (!class_exists('OVIC_API_v3_Client')){
	class OVIC_API_v3_Client
	{
		/**
		 * @var string
		 */
		...
		private function request( $method, $resource, array $data = array() )
		{
			...
			if ( empty( $this->api_key ) ) {
				throw new OVIC_API_Exception( "Missing API key", 001 );
			}
			 ...
			 * @param array $args
			 */
			$args = apply_filters( 'ovic_http_request_args', $args, $url );
		...
		private function get_headers()
		{
			...
			$headers                  = array();
			$headers['Authorization'] = 'Basic ' . base64_encode( 'ovic:' . $this->api_key );
			$headers['Accept']        = 'application/json';
			$headers['Content-Type']  = 'application/json';
			$headers['User-Agent']    = 'ovic-toolkit/' . OVIC_TOOLKIT_VERSION . '; WordPress/' . $wp_version . '; ' . get_bloginfo( 'url' );
		 ...
		 * @return mixed
		 *
		 * @throws OVIC_API_Connection_Exception|OVIC_API_Resource_Not_Found_Exception|OVIC_API_Exception
		 */
		private function parse_response( $response )
		{
			if ( $response instanceof WP_Error ) {
				throw new OVIC_API_Connection_Exception( $response->get_error_message(), (int)$response->get_error_code(), $this->last_request );
			}
			...
			if ( $code >= 400 ) {
				...
				if ( is_object( $data ) && isset( $data->type ) && $data->type === 'akamai_error_message' ) {
					throw new OVIC_API_Connection_Exception( $message, $code, $this->last_request, $this->last_response, $data );
				}

				if ( $code === 404 ) {
					throw new OVIC_API_Resource_Not_Found_Exception( $message, $code, $this->last_request, $this->last_response, $data );
				}

				// mailchimp returned an error..
				throw new OVIC_API_Exception( $message, $code, $this->last_request, $this->last_response, $data );
			}

			// unable to decode response
			if ( is_null( $data ) ) {
				throw new OVIC_API_Exception( $message, $code, $this->last_request, $this->last_response );
			}
