jQuery(function ($) {
    ...
    var initTab = function () {
        $(document).on('click', '.ovic-mapper ul.nav li', function (e) {
    ...
    var imageSelector = function () {
        $(document).on('click', '.ovic-mapper a.btn-add-image, .ovic-mapper #change-image, .ovic-mapper a.image-selector', function (event) {
            ...
            if (!window.wr_image_selector) {
                ...
                window.wr_image_selector = wp.media({
                    button: {
                        text: ovic_mapper.text.img_selector_btn_label,
                    },
                    states: [new wp.media.controller.Library({
                        title: ovic_mapper.text.img_selector_modal_title,
                ...
                window.wr_image_selector.on('select', function () {
                    ...
                    if (window.wr_image_selector.input_element.attr('id') == 'ovic_mapper_image') {
                        window.wr_image_selector.input_element.val(attachment.attributes.id);

                        // Update edit area with new image.
                        if ($('.ovic-mapper-bot > .edit-image').length) {
                            $('.ovic-mapper-bot .image-wrap > img').attr('src', attachment.attributes.url);
                        } else {
                            var edit_html = $('#ovic_mapper_image_tmpl').text().replace('%URL%', attachment.attributes.url);

                            $('.ovic-mapper-bot > .add-image').replaceWith(edit_html);
            ...
            window.wr_image_selector.input_element = $(this).hasClass('image-selector') ? $(this).prev('input') : $('#ovic_mapper_image');
    ...
    var setupApplication = function () {
        ...
        var PinList = Backbone.Collection.extend({
            ...
            fetch: function () {
                if (window.ovic_mapper_pins && window.ovic_mapper_pins.length) {
                    this.add(window.ovic_mapper_pins);
                }
            },
        });
        ...
        var PinView = Backbone.View.extend({
            ...
            template: _.template($('#ovic_mapper_pin_tmpl').text()),
            ...
            render: function () {
                ...
                this.$el.find('[data-option]').each(function () {
                    ...
                    if ($(this).attr('name') != option[0]) {
                        if (option[3] !== undefined) {
                            $(this).attr('name', 'ovic_mapper_pins[' + self.index + '][' + option[1] + '][' + option[3] + ']');
                        } else {
                            $(this).attr('name', 'ovic_mapper_pins[' + self.index + '][' + option[1] + ']');
                        }
                    }
                    ...
                    if ($(this).hasClass('color-picker')) {
                        ...
                        setTimeout($.proxy(function () {
                            $(this).parent().parent().click(function () {
                                ...
                                // focused.
                                $('.ovic-mapper .icon-selector').each(function () {
                    ...
                    // Init icon selector if needed.
                        ...
                        $(this).addClass('hidden').after($('#ovic_mapper_icon_selector_tmpl').text().replace('%SELECTED%', $(this).val()));
                    }
                    ...
                    // Init product selector if needed.
                        ...
                        $(this).select2({
                            ...
                            ajax: {
                                url: ovic_mapper.product_selector.url,
                                dataType: 'json',
                                delay: 250,
                                data: function (terms) {
                                    return {
                                        term: terms,
                                        security: ovic_mapper.product_selector.security,
                                    };
                                },
                            ...
                            initSelection: function (element, callback) {
                                ...
                                if (id !== '') {
                                    $.ajax(ovic_mapper.product_selector.url + '&term=' + id + '&security=' + ovic_mapper.product_selector.security, {
                                        dataType: 'json',
                                    }).done(function (data) {
                                        callback({
                                            id: id,
                ...
                }).on('click', '.ovic-icon-list a', function (event) {
                    event.preventDefault();
            ...
            edit: function (event) {
                if (!$(event.target).parent().hasClass('ovic-pin') || this.just_dragged) {
                    return;
                }
                ...
                if (this.$el.hasClass('editing')) {
                    ...
                    if (!this.$el.data('ovic_mapper_pin_settings_initialized')) {
                        var self = this;
                        ...
                        this.$el.data('ovic_mapper_pin_settings_initialized', true);
                    }
            ...
            close: function (event) {
                ...
                if (pin_type == 'woocommerce') {
                    ...
                    this.$el.find('.tooltip').text(title ? title : ovic_mapper.text.please_input_a_title).removeClass('hidden');
                }
            ...
            remove: function (event) {
                ...
                if (confirm(ovic_mapper.text.confirm_removing_pin)) {
                    this.$el.remove();
                    this.model.destroy();
        ...
        var PinListView = Backbone.View.extend({
            ...
            el: '.ovic-mapper-bot > .edit-image > .image-wrap',
            ...
            initialize: function () {
                ...
                $(document).on('click', '.ovic-mapper-bot > .edit-image > .image-wrap > img', this.create);
        ...
        $(document).on('init_ovic_mapper', function (event) {
            if (!$(document).data('ovic_mapper_settings_initialized')) {
                ...
                $('#general-settings').find('input, select, textarea').each(function () {
                    if (window.ovic_mapper_settings[$(this).attr('name')]) {
                        $(this).val(window.ovic_mapper_settings[$(this).attr('name')]);

                        if ($(this).attr('type') == 'hidden' && $(this).next().attr('type') == 'checkbox') {
                            if (parseInt(window.ovic_mapper_settings[$(this).attr('name')])) {
                                $(this).next().attr('checked', 'checked');
                            } else {
                                $(this).next().removeAttr('checked');
                            }
                    ...
                    $(this).attr('name', 'ovic_mapper_settings[' + $(this).attr('name') + ']');
                ...
                $('#general-settings').on('change', 'input[type="radio"], select', function () {
                    var option = $(this).attr('name').match(/ovic_mapper_settings\[([^\]]+)\]/), value = $(this).val();
                ...
                $(document).click(function (event) {
                    ...
                    $('.ovic-mapper .wp-picker-holder .iris-picker').each(function () {
                    ...
                    $('#general-settings.editing, .ovic-pin.editing').each(function () {
                ...
                // Track data changes.
                var data_fields = '.ovic-mapper input, .ovic-mapper select, .ovic-mapper textarea';

                $(document).on('change', data_fields, function () {
                    if ($(this).attr('name') && $(this).attr('name').indexOf('ovic_mapper_') > -1) {
                        $('#post #publish').attr('data-changed', 'yes');
                    }
                });
                ...
                $(window).on('beforeunload', function () {
                    ...
                    if (submit_btn.length && submit_btn.attr('data-changed') == 'yes') {
                        return ovic_mapper.text.ask_for_saving_changes;
                    }
                });

                $(document).data('ovic_mapper_settings_initialized', true);
            }

            if (!window.ovic_mapper_pins_app && $('.ovic-mapper-bot > .edit-image > .image-wrap').length) {
                // Init pin list view.
                window.ovic_mapper_pins_app = new PinListView;
            }
        });
    };
