...
if ( ! class_exists( 'Ovic_Attribute_Product_Widget' ) ) {
	class Ovic_Attribute_Product_Widget extends WC_Widget
	{
		...
		public function __construct()
		{
			...
			$this->widget_description = esc_html__( 'Shows a custom attribute in a widget.', 'ovic-toolkit' );
			$this->widget_id          = 'ovic_woocommerce_layered_nav';
			$this->widget_name        = esc_html__( 'Ovic: Attribute Product', 'ovic-toolkit' );
			parent::__construct();
		}
		...
		{
			$defaults             = array(
				'title'        => esc_html__( 'Filter by', 'ovic-toolkit' ),
				'attribute'    => '',
				'display_type' => 'list',
				'query_type'   => 'and'
			);
            ...
            <div class="ovic_layered_container">
                <p>
                    <label for="<?php echo $this->get_field_id( 'title' ); ?>">
						<?php esc_html_e( 'Title:', 'ovic-toolkit' ); ?>
                    </label>
                    <input type="text" class="widefat ovic_layered_title"
                           id="<?php echo $this->get_field_id( 'title' ); ?>"
                           name="<?php echo $this->get_field_name( 'title' ); ?>"
                           value="<?php echo $instance['title']; ?>"/>
                </p>
                <p>
                    <label for="<?php echo $this->get_field_id( 'attribute' ); ?>">
						<?php esc_html_e( 'Product attribute:', 'ovic-toolkit' ); ?>
                    </label>
                ...
                <p>
                    <label for="<?php echo $this->get_field_id( 'display_type' ); ?>">
						<?php esc_html_e( 'Display type:', 'ovic-toolkit' ); ?>
                    </label>
                    <select class="widefat ovic_layered_display_type"
                        ...
                        <option value="dropdown" <?php echo esc_attr( $instance['display_type'] == 'dropdown' ? 'selected' : '' ); ?>><?php esc_html_e( 'Dropdown Style',
								'ovic-toolkit' ) ?></option>
                        <option value="color" <?php echo esc_attr( $instance['display_type'] == 'color' ? 'selected' : '' ); ?> <?php echo esc_attr( $attribute_array[ $attribute_default ]['type'] != 'box_style' ? 'disabled' : '' ) ?> ><?php esc_html_e( 'Color Style',
								'ovic-toolkit' ) ?></option>
                ...
                <p>
                    <label for="<?php echo $this->get_field_id( 'query_type' ); ?>">
						<?php esc_html_e( 'Query type', 'ovic-toolkit' ); ?>
                    </label>
                    <select class="widefat ovic_layered_query_type"
            ...
            <script type="text/javascript">
                jQuery( '.ovic_layered_attribute' ).on( 'change', function () {
                    var type = jQuery( this ).find( ':selected' ).data( 'type' ),
                        p    = jQuery( this ).closest( '.ovic_layered_container' );
                    if ( type === 'color' ) {
                        p.find( '.ovic_layered_display_type option[value="color"]' ).removeAttr( 'disabled' );
                    } else {
                        p.find( '.ovic_layered_display_type option[value="color"]' ).attr( 'disabled', 'disabled' );
                        if ( p.find( '.ovic_layered_display_type option[value="color"]' ).is( ':selected' ) ) {
                            p.find( '.ovic_layered_display_type option[value="list"]' ).prop( 'selected', true );
                        }
                    }
                } );
            </script>
		...
		public function init_settings()
		{
			...
			$this->settings = array(
				'title'        => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Filter by', 'ovic-toolkit' ),
					'label' => esc_html__( 'Title', 'ovic-toolkit' ),
				),
				'attribute'    => array(
					'type'    => 'select',
					'label'   => esc_html__( 'Attribute', 'ovic-toolkit' ),
					'options' => $attribute_array,
				),
				'display_type' => array(
					'type'    => 'select',
					'std'     => 'list',
					'label'   => esc_html__( 'Display type', 'ovic-toolkit' ),
					'options' => array(
						'list'     => esc_html__( 'List', 'ovic-toolkit' ),
						'dropdown' => esc_html__( 'Dropdown', 'ovic-toolkit' ),
					),
				),
				'query_type'   => array(
					'type'    => 'select',
					'std'     => 'and',
					'label'   => esc_html__( 'Query type', 'ovic-toolkit' ),
					'options' => array(
						'and' => esc_html__( 'AND', 'ovic-toolkit' ),
						'or'  => esc_html__( 'OR', 'ovic-toolkit' ),
					),
				),
			);
		}
		...
		protected function layered_nav_dropdown( $terms, $taxonomy, $query_type )
		{
			...
			if ( $taxonomy !== $this->get_current_taxonomy() ) {
				...
				echo '<option value="">' . sprintf( esc_html__( 'Any %s', 'ovic-toolkit' ),
				...
				jQuery( '.dropdown_layered_nav_" . esc_js( $taxonomy_filter_name ) . "' ).change( function() {
					...
					location.href = '" . preg_replace( '%\/page\/[0-9]+%', '',
						str_replace( array( '&amp;', '%2C' ), array(
							'&',
							','
						), esc_js( add_query_arg( 'filtering', '1',
		...
		protected function layered_nav_list( $terms, $taxonomy, $query_type )
		{
			...
			foreach ( $terms as $term ) {
				...
				echo '<li class="wc-layered-nav-term ' . ( $option_is_set ? 'current-cat selected' : '' ) . '">';
				echo ( $count > 0 || $option_is_set ) ? '<a href="' . esc_url( apply_filters( 'woocommerce_layered_nav_link', $link, $term, $taxonomy ) ) . '">' : '<span>';
		...
		{
			global $ovic_toolkit, $woocommerce;
			// List display
			?>
				...
				foreach ( $terms as $term ) {
					...
					if ( $type == 'photo' ) {
						...
						if ( $thumbnail_id ) {
							...
							?>
                            <a class="term-color <?php if ( $option_is_set ): ?>current-cat selected<?php endif; ?>"
							...
							$color = get_term_meta( $term->term_id, $term->taxonomy . '_' . $ovic_toolkit->product_attribute_color . '_color', true );
							?>
                            <a class="term-color <?php if ( $option_is_set ): ?>current-cat selected<?php endif; ?>"
                               href="<?php echo esc_url( $link ); ?>">
	...
	add_action( 'widgets_init', 'Ovic_Attribute_Product_Widget' );

	function Ovic_Attribute_Product_Widget()
	{
