...
if ( !class_exists( 'OVIC_Fields' ) ) {
	abstract class OVIC_Fields extends OVIC_Abstract
	{
		...
		{
			$out = ( isset( $this->field['info'] ) ) ? '<p class="ovic-text-desc">' . $this->field['info'] . '</p>' : '';
		...
		public function element_debug()
		{
			...
			if ( !empty( $this->where ) && ( !empty( $this->field['debug'] ) || ( defined( 'OVIC_DEBUG' ) && OVIC_DEBUG ) ) ) {
				...
				$out   .= "<pre>";
				$out   .= "<strong>" . __( 'CONFIG', 'ovic-toolkit' ) . ":</strong>";
				$out   .= "\n";
				ob_start();
				...
				if ( !empty( $this->field['id'] ) ) {
					$out .= "\n";
					$out .= "<strong>" . __( 'ID', 'ovic-toolkit' ) . ":</strong>";
					$out .= "\n";
					$out .= $this->field['id'];
					$out .= "\n";
					$out .= "\n";
					$out .= "<strong>" . __( 'USAGE', 'ovic-toolkit' ) . ":</strong>";
					$out .= "\n";
					...
					if ( isset( $value ) ) {
						$out .= "\n\n";
						$out .= "<strong>" . __( 'VALUE', 'ovic-toolkit' ) . ":</strong>";
						$out .= "\n";
						ob_start();
						var_export( $value );
						$out .= htmlspecialchars( ob_get_clean() );
		...
		public function element_get_error()
		{
			global $ovic;
			$out = '';
			if ( !empty( $ovic['errors'] ) ) {
				foreach ( $ovic['errors'] as $key => $value ) {
					if ( isset( $this->field['id'] ) && $value['code'] == $this->field['id'] ) {
						$out .= '<p class="ovic-text-warning">' . $value['message'] . '</p>';
					}
				}
			}
		...
		{
			return ( isset( $this->field['help'] ) ) ? '<span class="ovic-help" data-title="' . $this->field['help'] . '"><span class="fa fa-question-circle"></span></span>' : '';
		}
			...
			if ( is_array( $this->multilang ) ) {
				...
				foreach ( $this->multilang['languages'] as $key => $val ) {
					...
					if ( $key != $this->multilang['current'] ) {
						...
						$class               = 'OVIC_Field_' . $this->field['type'];
				...
				$out .= '</fieldset>';
				$out .= '<p class="ovic-text-desc">' . sprintf( __( 'You are editing language: ( <strong>%s</strong> )', 'ovic-toolkit' ), $this->multilang['current'] ) . '</p>';
			}
 ...
 *
 */
if ( !function_exists( 'ovic_load_field_classes' ) ) {
	function ovic_load_field_classes()
	{
		foreach ( glob( OVIC_OPTIONS_DIR . '/fields/*/*.php' ) as $field ) {
			OVIC::locate_template( 'fields/' . wp_basename( dirname( $field ) ) . '/' . wp_basename( $field ) );
		}
		do_action( 'ovic_load_field' );
	}
}
// load all of fields
ovic_load_field_classes();
